...
from voltatrees.volta_XGBM import compiler
from voltatrees.volta_XGBM.data_processing import (
    data_to_ndarray,
    extract_model_global_features,
    extract_pandas_traintime_categories,
    ndarray_to_ptr,
)
from voltatrees.volta_XGBM.llvm_binding import compile_module_to_asm
...
class Model:
    ...
    def predict(self, data, n_jobs=None):         # os.cpu_count() only for very large chunks.
        if n_jobs==None:
            n_jobs=min(os.cpu_count()*data.shape[0]//100, os.cpu_count())
        """
